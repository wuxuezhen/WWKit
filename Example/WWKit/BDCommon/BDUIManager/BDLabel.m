...
static NSString *JMRange = @"llrange";
static NSString *JMColor = @"llcolor";
...
- (void)setup{
    ...
    _jm_textColor = [UIColor colorWithRed:162.0/255 green:162.0/255  blue:162.0/255  alpha:162.0/255];
    _jm_textHightLightBackGroundColor = [UIColor colorWithWhite:0.7 alpha:0.2];
...
- (void)prepareText{
    ...
    attr = @{
             ...
             NSForegroundColorAttributeName:self.jm_textColor
             };
    ...
    for (NSValue *value in topicRanges) {
        ...
        if (self.topicFont) {
            [self.textStorage addAttributes:@{
                                             NSFontAttributeName:self.topicFont,
                                             NSForegroundColorAttributeName:self.topicHightColor
                                             }
                                      range:range];
        }else{
    ...
    if (self.jm_matchArr.count > 0) {
        ...
        if (userDefineRangeDicts.count > 0) {
            ...
            for (NSDictionary *dict in userDefineRangeDicts) {
                NSValue *value = dict[JMRange];
                [arrM addObject:value];
                UIColor *color = dict[JMColor];
                NSRange range;
                [value getValue:&range];
...
- (void)setJm_textColor:(UIColor *)jm_textColor{
    _jm_textColor = jm_textColor;
    [self prepareText];
}

- (void)setJm_matchArr:(NSArray<NSDictionary *> *)jm_matchArr{
    _jm_matchArr = jm_matchArr;
    [self prepareText];
}
...
- (void)drawRect:(CGRect)rect{
    ...
    if (self.selectedRange.length != 0) {
        ...
        // 2 确定颜色
        UIColor *selectedColor = self.isSelected ? self.jm_textHightLightBackGroundColor :[UIColor clearColor];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    switch (self.tapStyle) {
        case HandleStyleAgreement:{
            ...
            if (self.handleTapBlock) {
                ...
                if (StrongSelf.handleTapBlock) {
                    StrongSelf.handleTapBlock(StrongSelf, HandleStyleAgreement, selectedString, StrongSelf.selectedRange);
                }
            }
        }
            break;
        case HandleStyleLink:{
            ...
            if (self.handleTapBlock) {
                ...
                if (!StrongSelf) return;
                if (StrongSelf.handleTapBlock) {
                    StrongSelf.handleTapBlock(StrongSelf, HandleStyleLink, selectedString, StrongSelf.selectedRange);
                }
            }
        }
            break;
        case HandleStyleTopic:{
            ...
            if (self.handleTapBlock) {
                ...
                if (!StrongSelf) return;
                if (StrongSelf.handleTapBlock) {
                    StrongSelf.handleTapBlock(StrongSelf, HandleStyleTopic, selectedString, StrongSelf.selectedRange);
                }
            }
        }
            break;
        case HandleStyleUser:{
            ...
            if (self.handleTapBlock) {
                ...
                if (!StrongSelf) return;
                if (StrongSelf.handleTapBlock) {
                    StrongSelf.handleTapBlock(StrongSelf, HandleStyleUser, selectedString, StrongSelf.selectedRange);
                }
            }
        }
            break;
        case HandleStyleUserDefine:{
            ...
            if (self.handleTapBlock) {
                ...
                if (!StrongSelf) return;
                if (StrongSelf.handleTapBlock) {
                    StrongSelf.handleTapBlock(StrongSelf, HandleStyleUserDefine, selectedString, StrongSelf.selectedRange);
                }
            }
        }
            break;
    ...
    // 4 代理
    if ([self.delegate respondsToSelector:@selector(bd_label:didSelectedString:forStyle:inRange:)]) {
        [self.delegate bd_label:self didSelectedString:selectedString forStyle:self.tapStyle inRange:self.selectedRange];
    }
...
-(NSArray<NSDictionary*> *)getUserDefineStringsRange{
    ...
    for (NSDictionary *dict in self.jm_matchArr) {
        ...
        aDictM[JMRange] = value;
        aDictM[JMColor] = color;
...
-(NSMutableAttributedString *)addLineBreak:(NSAttributedString *)attrString{
    ...
    if (paragraphStyle != nil) {
        ...
        if (self.spacing > 0) {
            [paragraphStyle setLineSpacing:self.spacing];
        }
    }else{
